...
function MapComponent() {
  ...
  useEffect(() => {
    if (!mapRef.current) {
      // stvaranje karte samo ako ref nije postavljen (MapComponent already initialized error)
      const map = L.map("map").setView([45.8, 15.967], 13); //stvaranje mape s centrom na Zagrebu

      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        //dodavanje plocice karti
        maxZoom: 19,
      }).addTo(map);

      var customIcon = L.divIcon({
        //ikona za prikaz markera
        html: `<div class="marker-container">
                 <img class="marker-icon" src="https://picsum.photos/20/30
                 " />
               </div>`,
      ...
      const createMarker = (lat, lng) => {
        //funkcija prikazivanja markera na temelju koordinata
        ...
        marker.on("click", () => {
          //otvaranje popUp-a svaki put kad se klikne na marker
          if (popupIsOpen) {
            marker.closePopup();
            popupIsOpen = false;
          } else {
      ...
      var jason =
        '[{"lat":45.800, "lng" :15.967},{"lat":45.800, "lng":15.969}, {"lat":45.801, "lng":15.971}]';
  ...
  return <div id="map" style={{ width: "90%", height: "70vh" }}></div>;
}
